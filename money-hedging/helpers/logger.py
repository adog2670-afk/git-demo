...
class TradingLogger:
    ...
    def _setup_logger(self, log_to_console: bool) -> logging.Logger:
        ...
        class UnbufferedFileHandler(logging.FileHandler):
            """File handler that flushes immediately for nohup compatibility."""
            def emit(self, record):
                try:
                    msg = self.format(record)
                    self.stream.write(msg + self.terminator)
                    self.stream.flush()  # Force immediate flush
                except Exception:
        ...
        # File handler with immediate flush for nohup compatibility
        file_handler = UnbufferedFileHandler(self.debug_log_file, mode='a', encoding='utf-8')
