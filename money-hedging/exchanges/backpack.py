...
class BackpackClient(BaseExchangeClient):
    ...
    async def place_open_order(self, contract_id: str, quantity: Decimal, direction: str,for_trigger: bool=False, protect_price=False) -> OrderResult:
        ...
        while retry_count < max_retries:
            ...
            if direction == "buy":
                # For buy orders, place slightly below best ask to ensure execution
                order_price = best_ask - self.config.tick_size
                if protect_price and best_bid + self.config.tick_size < order_price:
                    order_price = best_bid + self.config.tick_size
                side = "Bid"
            else:
                # For sell orders, place slightly above best bid to ensure execution
                order_price = best_bid + self.config.tick_size
                if protect_price and best_ask - self.config.tick_size > order_price:
                    order_price = best_ask - self.config.tick_size
                side = "Ask"
            
            
            self.logger.log(f"[OPEN] Order calculated, best_bid:{best_bid}, best_ask:{best_ask}, protect_price:{protect_price}, order_price:{order_price}", "WARNING")
            ...
            try:
                ...
                # Backpack occasionally closes TLS connections abruptly; retry instead of failing the strategy
                self.logger.log(
                    f"[OPEN] Ignoring SSL error while placing order: {ssl_error}",
                    "WARNING",
                )
            ...
            self.logger.log(
                f"[OPEN] Created order {order_id} side={side.lower()} qty={quantity} price={self.round_to_tick(order_price)} protect_price={protect_price}",
                "INFO"
            )
    ...
    async def place_market_order(self, contract_id: str, quantity: Decimal, direction: str) -> OrderResult:
        ...
        # For market orders, we expect them to be filled immediately
            ...
            self.logger.log(
                f"[MARKET] Filled order {order_id} side={direction.lower()} qty={quantity} price={price}",
                "INFO"
            )
            return OrderResult(
                success=True,
    ...
    async def place_close_order(self, contract_id: str, quantity: Decimal, price: Decimal, side: str) -> OrderResult:
        ...
        while retry_count < max_retries:
            ...
            self.logger.log(
                f"[CLOSE] Created order {order_id} side={side.lower()} qty={quantity} price={adjusted_price}",
                "INFO"
            )
    ...
    async def cancel_order(self, order_id: str) -> OrderResult:
        ...
        try:
            ...
            if 'code' in cancel_result:
                ...
                self.logger.log(f"[CLOSE] Canceled order {order_id}", "INFO")
    ...
    async def get_account_balance(self) -> Decimal:
        ...
        try:
            ...
            balance_data = self.account_client.get_balance()

            if not balance_data:
                self.logger.log("No balance data received from Backpack", "WARNING")
                return Decimal("0")

            # Log the balance data structure for debugging
            self.logger.log(f"Balance data structure: {type(balance_data)} - {balance_data}", "DEBUG")

            # Try to extract USDC balance from different possible fields
            if isinstance(balance_data, dict):
                # Look for USDC balance in dictionary
                balance_fields = [
                    'available', 'usdc_available', 'USDC', 'usdc',
                    'balance', 'usdc_balance', 'available_balance'
                ]

                for field in balance_fields:
                    if field in balance_data:
                        balance_value = balance_data[field]
                        if isinstance(balance_value, (int, float, str, Decimal)):
                            balance = Decimal(str(balance_value))
                            self.logger.log(f"Found USDC balance in field '{field}': {balance}", "INFO")
                            return balance

                # Check for nested balance structure
                        ...
                        for balance_item in balances:
                            if isinstance(balance_item, dict):
                                ...
                                if asset == 'USDC':
                                    ...
                                    self.logger.log(f"Found USDC balance in balances list: {balance}", "INFO")
                                    return balance

                # If no USDC balance found, log available fields
                available_fields = list(balance_data.keys())
                self.logger.log(f"Available balance fields: {available_fields}", "DEBUG")

                # Try to find any numeric field that could be balance
                for key, value in balance_data.items():
                    if isinstance(value, (int, float, Decimal)) and value > 0:
                        self.logger.log(f"Potential balance field '{key}': {value}", "DEBUG")
                ...
                for item in balance_data:
                    if isinstance(item, dict):
                        ...
                        if asset == 'USDC':
                            ...
                            balance = Decimal(str(available))
                            self.logger.log(f"Found USDC balance in list: {balance}", "INFO")
                            return balance

            # If we can't find the balance, return 0
            self.logger.log("USDC balance not found in Backpack balance data", "WARNING")
            return Decimal("0")

        except Exception as e:
            self.logger.log(f"Error fetching Backpack account balance: {e}", "ERROR")
            import traceback
