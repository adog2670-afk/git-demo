...
class LighterClient(BaseExchangeClient):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger = TradingLogger(
            exchange="lighter", ticker=self.config.ticker, log_to_console=False
        )
    ...
    def _handle_websocket_order_update(self, order_data_list: List[Dict[str, Any]]):
        ...
        for order_data in order_data_list:
            ...
            if order_id in self.orders_cache.keys():
                if (
                    self.orders_cache[order_id]["status"] == "OPEN"
                    and status == "OPEN"
                    and filled_size == self.orders_cache[order_id]["filled_size"]
                ):
                    continue
            ...
            if status == "OPEN":
                self.logger.log(
                    f"[{order_type}] [{order_id}] {status} {size} @ {price}", "INFO"
                )
            else:
                self.logger.log(
                    f"[{order_type}] [{order_id}] {status} {filled_size} @ {price}",
                    "INFO",
                )

            if (
                order_data["client_order_index"] == self.current_order_client_id
                or order_type == "OPEN"
            ):
    ...
    async def fetch_bbo_prices(self, contract_id: str) -> Tuple[Decimal, Decimal]:
        ...
        if (
            ...
            and self.ws_manager.best_bid
            and self.ws_manager.best_ask
        ):
    ...
    async def _submit_order_with_retry(
        ...
        if error is not None:
            ...
            return OrderResult(
                success=True, order_id=str(order_params["client_order_index"])
            )
    ...
    async def place_limit_order(
        ...
        if order_result.success:
            self.logger.log(
                f"[LIMIT] Created order {order_result.order_id} side={side.lower()} qty={quantity} price={price}",
                "INFO",
            )
        return order_result
    ...
    async def place_market_order(
        ...
        """Place a market (IOC) order using the official Lighter SDK."""
        if self.lighter_client is None:
            await self._initialize_lighter_client()
        ...
        is_ask = side == "sell"
        base_amount_decimal = (
            quantity * Decimal(self.base_amount_multiplier)
        ).to_integral_value(rounding=ROUND_HALF_UP)
        base_amount = int(base_amount_decimal)
        if base_amount <= 0:
            raise ValueError(f"Calculated base amount is non-positive: {base_amount}")

        max_slippage = 0.005  # 千分之5滑点
        ...
        (
            ...
            tx_hash,
            error,
        ) = await self.lighter_client.create_market_order_limited_slippage(
        ...
        self.logger.log(
            f"place_market_order success: {create_order.to_json()}",
            "INFO",
        )

        # executed_price = Decimal(avg_execution_price) / Decimal(self.price_multiplier)
        # executed_size = Decimal(base_amount) / Decimal(self.base_amount_multiplier)

        executed_price = Decimal(create_order.price) / Decimal(self.price_multiplier)
        executed_size = Decimal(base_amount) / Decimal(self.base_amount_multiplier)

        self.logger.log(
            f"[MARKET] Submitted IOC order {client_order_index} side={side} qty={executed_size} price={executed_price}",
            "INFO",
        )
    ...
    async def place_close_order(
        ...
        if order_result.success:
            ...
            raise Exception(
                f"[CLOSE] Error placing order: {order_result.error_message}"
            )
    ...
    async def cancel_order(self, order_id: str) -> OrderResult:
        ...
        if error is not None:
            return OrderResult(
                success=False, error_message=f"Cancel order error: {error}"
            )
        ...
        if tx_hash:
            ...
            return OrderResult(
                success=False, error_message="Failed to send cancellation transaction"
            )
    ...
    async def get_account_positions(self) -> Decimal:
        ...
        for position in positions:
            if position.market_id == self.config.contract_id:
                return Decimal(position.position) * Decimal(position.sign)
    ...
    async def get_account_balance(self) -> Decimal:
        ...
        try:
            # Use the same API call as _fetch_positions_with_retry
            import lighter
            account_api = lighter.AccountApi(self.api_client)

            # Get account info which includes balance information
            account_data = await account_api.account(
                by="index", value=str(self.account_index)
            )
            ...
            if account_data and account_data.accounts:
                ...
                self.logger.log(f"Account attributes: {[attr for attr in dir(account) if not attr.startswith('_')]}", "DEBUG")

                # Try different possible balance fields
                balance_fields = [
                    'cash_balance', 'balance', 'available_balance',
                    'total_balance', 'usdc_balance', 'settled_balance'
                ]

                for field in balance_fields:
                    if hasattr(account, field):
                        balance_value = getattr(account, field)
                        self.logger.log(f"Found balance field '{field}': {balance_value}", "INFO")
                        return Decimal(str(balance_value))

                # If no standard balance field found, check raw data
                if hasattr(account_data, 'raw_data') and account_data.raw_data:
                    raw_data = account_data.raw_data
                    self.logger.log(f"Raw account data: {raw_data}", "DEBUG")

                    # Look for balance in raw data
                    if isinstance(raw_data, dict):
                        for key, value in raw_data.items():
                            if 'balance' in key.lower() and isinstance(value, (int, float, str)):
                                self.logger.log(f"Found balance in raw data '{key}': {value}", "INFO")
                                return Decimal(str(value))

                # Last resort: check if there's any numeric field that could be balance
                for attr in dir(account):
                    if not attr.startswith('_'):
                        try:
                            value = getattr(account, attr)
                            if isinstance(value, (int, float, Decimal)) and value > 0:
                                self.logger.log(f"Potential balance field '{attr}': {value}", "DEBUG")
                        except:
                            pass

                self.logger.log("No balance field found in lighter account data", "WARNING")
                return Decimal("0")
            else:
                self.logger.log("No account data available for balance", "WARNING")
                return Decimal("0")
    ...
    async def get_contract_attributes(self) -> Tuple[str, Decimal]:
        ...
        try:
            self.config.tick_size = Decimal("1") / (
                Decimal("10") ** order_book_details.price_decimals
            )
        except Exception:
